...
# Inline `<code class="docutils literal notranslate">…</code>` walker.
# Covers what neither the markdown-time step 8g pass-1 nor the
# `<pre>`-scoped walker above reach:
#   • Markdown backtick spans in prose (`` `Mat` `` inside a description /
#     a `**Parameters**` blurb / a brief sentence) — MyST only turns those
#     into `<code>` HTML at render time, AFTER step 8g has run.
#   • Typedef and Variable Type-column code spans on api pages other
#     than `core_basic` (step 8b's Type-column rewrite is core_basic-only).
#
# Idempotent against pages step 8g already linkified: existing `<a>`
# regions site-wide are masked out before the inline `<code>` scan, so
# tokens already wrapped in an anchor (or `<code>` blocks nested inside
# an outer `<a>`) stay untouched — no double-anchoring, no nested `<a>`.
...
def _linkify_inline_code(html_dir: pathlib.Path) -> None:
    """Wrap recognised `cv::*` type tokens inside `<code class="docutils
    literal notranslate">` inline elements in an `<a class="reference
    internal" href="…">` when the token resolves via
    `_LOCAL_CLASS_URL` / `_LOCAL_TYPEDEF_URL`. Only `<code>` bodies that
    contain NO existing markup (`<` filtered by the regex) are
    rewritten, which naturally excludes Pygments-coloured `<code>`
    children and `<code>` blocks containing nested `<a>` (the step 8g
    Functions / Function-Documentation signatures)."""
    ...
    def _anchor_text(tok: str) -> str:
        # `::` → `&#58;&#58;` so any downstream cv-linkifier pass
        # doesn't nest a second anchor around `cv::Name`.
        return tok.replace("::", "&#58;&#58;")

    def _linkify_body(body: str) -> str:
        # Body is plain text (no nested tags); linkify recognised
        # tokens at boundaries via `re.sub`.
    ...
    for html in html_dir.rglob("*.html"):
        ...
        if 'class="docutils literal notranslate"' not in text:
            continue
        # Mask every `<a>…</a>` region so we don't double-link tokens
        # already anchored upstream and don't create nested `<a>` if a
        # whole `<code>` is wrapped in an outer link (e.g. `[`Mat`](url)`
        # markdown → `<a><code>Mat</code></a>` HTML).
        masked: list[str] = []
