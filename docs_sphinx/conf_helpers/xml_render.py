...
def _member_detail_parts(md):
    ...
    for ss in de.iter("simplesect"):
        if ss.get("kind") == "return":
            # Block-aware conversion — same treatment we give parameter
            # descriptions above. Using `_itertext` here flattened the
            # simplesect to plain text, dropping <ref> cross-references,
            # <ulink> external links, <computeroutput> inline code,
            # <formula> math, and <itemizedlist> bullets — the live
            # page's richer Returns content vanished.
